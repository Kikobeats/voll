...
function createInsensitiveCompare (includes, lowercase) {
  function insensitiveCompare (node, vars) {
    var hasSpace = includes(node.token.value, ' ', { split: false })
    return includes(lowercase(vars), lowercase(node.token.value), {
      split: !hasSpace
    })
  }
...
function Evaluator (opts) {
  ...
  if (opts.cache !== false) {
    ...
    compare = opts.insensitive
      ? createInsensitiveCompare(includes, lowercase)
      : createSensitiveCompare(includes)
  }
